...
6. **Radar**: Leaflet map centered ~40.70,-74.02, zoom ~10. Base tiles: CARTO light/dark (auto per theme). RainViewer frames (past + nowcast) as animated overlay with play/pause + frame slider + timestamp label. Marker at Pier 25. Preserve every active NWS warning polygon; its official CAP motion vector is drawn as ONE dashed track anchored at the polygon centroid (mean of the first polygon's exterior-ring vertices), not one parallel track per motion-description vertex. Radar-cell tracks use client-side SCIT-style (Johnson et al. 1998, WSR-88D Build 9) centroid tracking: on each storm-cell refresh, fetch up to three past NEXRAD storm-attribute snapshots (now−10/20/30 min, rounded down to 5-minute buckets and cached per bucket in localStorage since a past snapshot never changes) to build each cell's position history (≤10 scans, associated by matching radar+storm_id or, failing that, the nearest cell within 10 km whose reported bearing is within 60°). Fit velocity by equal-weight linear least squares over that history, anchored at the latest observed centroid; fall back to the radar-reported motion when there are fewer than 3 history points, or when the fit disagrees with the reported motion by more than 2x speed or 60° heading (mis-association guard). Cap displayed radar-cell tracks at four, ranked first by closest projected one-hour approach to Pier 25 and then by meteorological significance and recency. Represent forecast uncertainty as whiskers perpendicular to the track: length is the 99%-probability diameter for that lead time (interpolated 1.5/3.6/5.8/8.0 nm at 15/30/45/60 min), scaled by a consistency factor from the fit's own residual spread (narrower when the track has been steady, wider when erratic; a fixed middle-ground factor applies with no usable fit), clamped to 0.6-10 nm. Keep 15-minute ticks, but permanently label only each track's +60-minute endpoint (warning tracks too); intermediate times appear on hover only. Draw each cell's observed history as a thin, translucent solid line behind its forecast track. While the radar loop plays (past frames + nowcast), the full track for each cell/warning-with-motion — line, whiskers, timestamp labels, and the observed-history trail, plus a small filled position marker — re-anchors to the storm's position at the displayed frame time: at/before its latest observation the position is interpolated along observed history, beyond it the position is extrapolated forward from that observation along its motion (clamped at a warning's expiration), and the forecast spread is rebuilt from there (cells always re-project a full 60-minute window from the displayed frame; warnings clamp at their end time and collapse to a single point once the frame passes it), with whisker widths sized by lead time measured from the displayed frame rather than from the latest scan.
...
- A green GO requires fresh NWS wind, hourly forecast, and advisory availability. Missing critical inputs produce CAUTION — DATA UNAVAILABLE; only a successfully loaded empty alert list means no active advisories. A critical feed still confirming its first load is not yet missing: an otherwise-green verdict shows CAUTION — CHECKING LIVE DATA until the fetch succeeds or actually fails.
