...
- A green GO requires fresh NWS wind, hourly forecast, and advisory availability. Missing critical inputs produce CAUTION — DATA UNAVAILABLE; only a successfully loaded empty alert list means no active advisories.
- Current curve and MAX_SLACK timing are independent capabilities: a complete curve still drives Direction when timing is unavailable; incomplete curves may show only in-range departure facts and never a route heading. Current-feed failures use a compact status and deduplicated 30 s / 60 s / 120 s automatic retries instead of blanking Direction.
