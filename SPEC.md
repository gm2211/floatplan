...
- Operational ferry traffic is part of the same one-minute, two-dimensional integrator and turn-around solve. Expand published departures for NY Waterway's Hoboken/NJT↔Brookfield route (weekday/weekend) and Hoboken 14th↔Brookfield route (weekday only) from a dated checked-in snapshot of the operator's official pages. Project the real terminals into the N11 along/cross frame, interpolate each crossing, and use continuous relative-motion closest-point-of-approach rather than distances at minute endpoints.
- When a scheduled crossing approaches within the conservative CPA limit, deterministically yield by altering to pass astern when the sailed heading permits it, or luff/slow while the ferry passes ahead. Re-evaluate every modeled trip on every step; retain a response only while its conflict remains, and never declare a ferry cleared after a fixed timer. This is an operational yield, never a fake tack/jibe; it must alter the integrated path/time and therefore the solved turn-around. If no candidate preserves the modeled 0.20 nm clearance, label the conflict unresolved and tell the skipper to slow/stop and reassess visually.
- Always identify ferry positions as a **Scheduled ferry estimate ±2 min**, link the two official NY Waterway schedule pages, show the snapshot date, and say explicitly that this is not live AIS. Sweep the entire timing interval with a conservative relative-motion error bound; endpoints and the on-time case alone are insufficient. A direct reach with no conflict remains straight.
- Make the sailed geometry auditable in playback without altering it for visual effect. Render sparse deterministic streamlines moving toward the forecast wind flow (`wind from + 180°`), a heading-oriented overhead hull, and a translucent no-sail sector centered on the forecast wind-from bearing with the same ±47° boundary used by the sailing polar. The flow phase follows simulator time so scrubbing is repeatable, and freezes when reduced motion is requested. Hide the sector when wind is unavailable or the boat is moored/overdue.
- Solve return feasibility in two dimensions. A ferry detour must be recovered with an integrated centerline intercept when the wind permits; the boat is home only when the actual path enters the Pier 25 along/cross mooring circle. Never snap an off-center latitude crossing or a moored tail to `crossNm = 0`.
